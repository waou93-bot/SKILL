...
When this skill is applied to a themed site or active project, invoke `toolbox` before finalizing the Design Read. Use the returned domain references, UX observations, technical candidates, media provenance and rejected options as evidence. Keep the Design Read responsible for the visual direction: `toolbox` supplies material for judgment, not a preset layout or a component library.
