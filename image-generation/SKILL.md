...
## Continuity for related images

When the request contains two or more related images, recurring subjects, variants, revisions, or assets that will be reused in another medium, load `asset-continuity` before writing prompts. Create or reuse its continuity pack, assign stable IDs, and build every prompt from the same canonical base plus a local delta. A standalone image with no recurring element does not need that workflow.

Reference the canonical source images directly whenever the tool permits. Do not rely only on the immediately previous generation: this creates cumulative drift. After each coherent batch, compare the outputs to the canon and to their logical neighbors, record state changes, and regenerate only the inconsistent assets.
...
- Continuity scope: Which subjects, brand elements, states, or style rules must remain fixed across the set
