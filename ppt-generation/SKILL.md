...
Load `asset-continuity` for every multi-slide deck. Register the deck's palette, typography, grid, logo, illustration language, recurring subjects and approved master slide references. Treat slide-to-slide reference chaining only as a local aid: every slide must also remain anchored to the canonical deck rules so small errors do not accumulate.
...
2. **For the FIRST slide (slide 1)**, create a prompt that establishes the visual style and register the approved result as the deck's canonical visual reference:
...
3. **For subsequent slides (slide 2+)**, use the PREVIOUS slide as a local reference image and the canonical master reference or style pack as the stable reference. Never depend on the previous slide alone:
...
4. **Continue for all remaining slides**, referencing the previous slide plus the canonical master whenever the provider supports multiple references:
...
- Keep the canonical master slide or approved style reference in the chain as well; the previous slide alone is not a source of truth
...
- Inspect a contact sheet of the full deck against the `asset-continuity` canon before composition
