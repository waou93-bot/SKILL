...
> **asset-continuity**: Load `asset-continuity` when the video contains recurring product UI, brand marks, screenshots, generated media, voices, treatments, or elements that cross frame boundaries. Its stable IDs and canon complement the capture inventory and `media-use` ledger; do not create a competing asset list.
...
Map the usable inventory entries to `asset-continuity` IDs and record the approved logo, tokens, screenshots, product states, voices and media treatments as canonical references. Preserve the capture paths; the continuity pack points to them rather than copying them.
...
Record each selected candidate's stable continuity ID in the frame block. When an asset changes state across the story, record the transition in the continuity state ledger and use the expected entry state in the outgoing and incoming frame specifications.
...
Ensure every packet contains the continuity IDs and locked values needed by that frame through its storyboard block and `frame.md`. Parallel workers must receive the same canonical logo, colors, typography, product state and recurring-element specification.
...
Compare the same contact sheet to the `asset-continuity` canon for brand, product UI, screenshots, recurring media and state changes. Record the result in `QA_CONTINUITY.md`; a passing seam check alone does not prove that the correct canonical asset was used.
