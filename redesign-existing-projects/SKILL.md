...
After the scan and before choosing upgrade techniques, invoke `toolbox` when the project has a defined domain or when a library, media source, interaction pattern, or replacement reference must be selected. Feed it the existing stack and constraints; use its shortlist to support the redesign without introducing an unrelated framework or copying a reference site.
