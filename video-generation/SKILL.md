...
## Continuity across clips and shots

For a multi-shot video, a set of clips, or any recurring character, product, location, brand element, voice, or evolving state, load `asset-continuity`. Register the canonical references and build every shot from the same canonical base plus its shot-specific delta. A single independent clip without recurring elements can stay on the direct workflow.

Do not use only the previous generated frame or clip as the next reference. Keep the canonical subject and style references attached whenever supported, record intentional state changes on the timeline, and compare boundary frames plus a representative contact sheet before delivery.
...
- Continuity scope: Recurring entities, locked design rules, and state changes between shots
