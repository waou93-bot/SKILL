...
Load `asset-continuity` for every video with multiple scenes or recurring visual, audio, brand, product, character, presenter, location, or state. Maintain stable IDs for source files and derivatives, keep canonical references attached to each generation, and record voice, palette, typography, treatment, dimensions, seeds or reusable parameters, and intentional state changes. A chained clip is a derivative, not a new source of truth.
...
Before generating, create or reuse the `asset-continuity` pack in the video project. Register the approved brand and subject references, presenter portrait, voice, music, scene images, clips and derived exports. Build scene prompts from the same canonical base plus a scene-specific delta.
...
4. Derive those anchors from the `asset-continuity` canon and reattach canonical subject or style references whenever the tool supports them; do not trust the previous clip alone
